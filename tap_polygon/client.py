...
class PolygonRestStream(RESTStream):
    ...
    def _to_datetime(ts: int | float | str | None) -> datetime | None:
        ...
        if isinstance(ts, str):
            try:
                ...
                logging.error(
                    f"Error converting timestamp string '{ts}' to datetime: {e}"
                )
    ...
    def _break_loop_check(self, context: Context) -> bool:
        ...
        if not record_timestamp_key:
            logging.info(
                f"No record timestamp key found for stream {self.name}. Continuing pagination."
            )
            return False
        ...
        if isinstance(record, list):
            ...
            if not timestamps_seen:
                logging.info(
                    f"No valid timestamps found in record list for {self.name}. Continuing."
                )
                return False
            ...
            logging.info(
                f"Record is not a dict or list for {self.name}. Cannot perform timestamp checks. Continuing."
            )
            return False
        ...
        if last_ts_dt is None:
            logging.warning(
                f"Could not parse last timestamp for stream {self.name}. Continuing."
            )
            return False
    ...
    def paginate_records(self, context: Context) -> t.Iterable[dict[str, t.Any]]:
        ...
        while True:
            ...
            if not records:
                logging.info(
                    f"No records returned for {self.name}. Breaking pagination."
                )
                break
            ...
            processed_records = (
                []
            )  # keeps only a list of records in single response and then resets to an empty list [].
            for record in records:
                if self._clean_in_place:
                    ...
                    cleaned_record = self.clean_record(
                        record, ticker=context.get("ticker")
                    )
            ...
            context.update(
                {
                    ...
                    "record_timestamp_key": self.get_record_timestamp_key(
                        processed_records
                    ),
                }
            )
    ...
    def _yield_records_for_tickers(
        self, context: Context
    ) -> t.Iterable[dict[str, t.Any]]:
        ticker_records = context.get("ticker_records")
        if not isinstance(ticker_records, list):
            raise ValueError(
                "When yielding records by ticker, context['ticker_records'] must be a list."
            )
